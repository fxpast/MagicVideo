...
- (void)viewDidLoad
{
    ...
    self.playerView.lumaThreshold =1;
    self.playerView.chromaThreshold =1;
...
- (void) LoadMovie:(id)sender From: (NSInteger) type {
...
- (IBAction)loadMovieFromCameraRoll:(id)sender
{
    ...
    [self LoadMovie:sender From:UIImagePickerControllerSourceTypePhotoLibrary];

}


- (IBAction)LoadMovieFromCamera:(id)sender {
    
    [self LoadMovie:sender From:UIImagePickerControllerSourceTypeCamera];
}
